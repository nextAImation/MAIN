    ...
    def _update_exit_logic(
        ...
        if pos_size > 0 and s.inited_l and not math.isnan(s.entry_price_l) and not math.isnan(s.atr_at_entry_l):
            ...
            if not exitWeakRawL:
                ...
                bars_since_not = math.nan if last_not_idx is None else idx - last_not_idx
            exitWeakL = (not math.isnan(bars_since_not)) and (bars_since_not >= cfg.weak_confirm_bars)
        ...
        if pos_size < 0 and s.inited_s and not math.isnan(s.entry_price_s) and not math.isnan(s.atr_at_entry_s):
            ...
            if not exitWeakRawS:
                ...
                bars_since_not_s = math.nan if last_not_idx_s is None else idx - last_not_idx_s
            exitWeakS = (not math.isnan(bars_since_not_s)) and (bars_since_not_s >= cfg.weak_confirm_bars)
