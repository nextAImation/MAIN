...
from order_manager import OrderManager, Order, OrderType, OrderSide, OrderStatus, Fill
...
class MatchingEngine:
    def __init__(self, order_manager: OrderManager, tick_size: Decimal = Decimal('0.01'),
                 maker_fee: Decimal = Decimal('0.001'), taker_fee: Decimal = Decimal('0.002'),
                 latency_ms: int = 0, slippage_mode: str = "none", 
                 fixed_slippage_ticks: Decimal = Decimal('0'), dynamic_coeff: Decimal = Decimal('1.0'),
                 queue_mode: QueueMode = QueueMode.FIFO, base_latency_ms: int = 0,
                 jitter_ms: int = 0, execution_latency_ms: int = 0, latency_bucket_ms: int = 0,
                 seed: int = 42):
    ...
    def _apply_latency(self, timestamp: datetime, order_id: str) -> datetime:
        ...
        if self.latency_bucket_ms > 0:
            bucket_ms = (new_time.microsecond // 1000) // self.latency_bucket_ms * self.latency_bucket_ms
    ...
    def _round_quantity_to_tick(self, quantity: Decimal) -> Decimal:
        if quantity is None:
            return Decimal('0')
        return (quantity / self.tick_size).to_integral_value(rounding=ROUND_FLOOR) * self.tick_size

    def _apply_slippage(self, fill_price: Decimal, fill_quantity: Decimal, side: OrderSide) -> Decimal:
        ...
        if self.slippage_mode == "fixed":
            ...
            slippage_ticks = (volume_ratio * self.dynamic_coeff).quantize(Decimal('1'), rounding=ROUND_FLOOR)
        else:
            return fill_price
    ...
    def _get_tradingview_sequence(self, open_price: Decimal, high: Decimal, low: Decimal, 
                                close: Decimal, direction: BarDirection) -> List[Tuple[Decimal, Decimal, Decimal]]:
    ...
    def _get_microtick_sequence(self, open_price: Decimal, high: Decimal, low: Decimal, 
                               close: Decimal, direction: BarDirection) -> List[Tuple[Decimal, Decimal, Decimal]]:
        sequence = []
        num_steps = 12
    ...
    def _create_fill_dict(self, order: Order, fill_price: Decimal, fill_quantity: Decimal,
                         current_time: datetime, is_maker: bool, order_type: OrderType) -> Dict[str, Any]:
    ...
    def _determine_aggressor(self, order1: Order, order2: Order) -> Tuple[Order, Order]:
    ...
    def _execute_limit_vs_limit_pro_rata(self, aggressor: Order, resting_orders: List[Order],
                                       fill_quantity: Decimal, current_time: datetime) -> List[Dict]:
        fills = []
        total_resting_quantity = sum(order.remaining_quantity for order in resting_orders)
        ...
        for resting_order in resting_orders:
            ...
            if fill_qty > Decimal('0'):
                ...
                self.order_manager.update_order_fill(aggressor.order_id, fill_price, 
                                                   fill_qty, execution_time, self.taker_fee, self.position)
                self.order_manager.update_order_fill(resting_order.order_id, fill_price, 
                                                   fill_qty, execution_time, self.maker_fee, self.position)
                
                self._update_position(aggressor, fill_qty, aggressor.side == OrderSide.BUY)
                self._update_position(resting_order, fill_qty, resting_order.side == OrderSide.BUY)
                
                aggressor_fill = self._create_fill_dict(aggressor, fill_price, fill_qty, 
                                                      execution_time, False, aggressor.order_type)
                resting_fill = self._create_fill_dict(resting_order, fill_price, fill_qty, 
                                                    execution_time, True, resting_order.order_type)
    ...
    def _execute_limit_vs_limit(self, aggressor: Order, resting: Order, 
                              fill_quantity: Decimal, current_time: datetime) -> Tuple[Dict, Dict]:
        ...
        self.order_manager.update_order_fill(aggressor.order_id, fill_price, 
                                           fill_quantity, execution_time, self.taker_fee, self.position)
        self.order_manager.update_order_fill(resting.order_id, fill_price, 
                                           fill_quantity, execution_time, self.maker_fee, self.position)
        
        self._update_position(aggressor, fill_quantity, aggressor.side == OrderSide.BUY)
        self._update_position(resting, fill_quantity, resting.side == OrderSide.BUY)
        
        aggressor_fill = self._create_fill_dict(aggressor, fill_price, fill_quantity, 
                                              execution_time, False, aggressor.order_type)
        resting_fill = self._create_fill_dict(resting, fill_price, fill_quantity, 
                                            execution_time, True, resting.order_type)
        
        return aggressor_fill, resting_fill

    def _execute_market_vs_limit(self, market_order: Order, limit_order: Order,
                               fill_quantity: Decimal, current_time: datetime) -> Tuple[Dict, Dict]:
        ...
        self.order_manager.update_order_fill(market_order.order_id, fill_price,
                                           fill_quantity, execution_time, self.taker_fee, self.position)
        self.order_manager.update_order_fill(limit_order.order_id, fill_price,
                                           fill_quantity, execution_time, self.maker_fee, self.position)
        
        self._update_position(market_order, fill_quantity, market_order.side == OrderSide.BUY)
        self._update_position(limit_order, fill_quantity, limit_order.side == OrderSide.BUY)
        
        market_fill = self._create_fill_dict(market_order, fill_price, fill_quantity,
                                           execution_time, False, market_order.order_type)
        limit_fill = self._create_fill_dict(limit_order, fill_price, fill_quantity,
                                          execution_time, True, limit_order.order_type)
        
        return market_fill, limit_fill

    def _execute_stop_market_fallback(self, order: Order, step_high: Decimal, step_low: Decimal,
        ...
        execution_price = self._apply_slippage(execution_price, fill_quantity, order.side)
        
        execution_time = self._apply_execution_latency(current_time)
        
        self.order_manager.update_order_fill(order.order_id, execution_price,
                                           fill_quantity, execution_time, self.taker_fee, self.position)
        
        self._update_position(order, fill_quantity, order.side == OrderSide.BUY)
        
        market_fill = self._create_fill_dict(order, execution_price, fill_quantity,
                                           execution_time, False, order.order_type)
        fills.append(market_fill)
        
        return fills

    def _process_stop_loss_orders(self, price_level: Decimal, step_high: Decimal, step_low: Decimal, current_time: datetime) -> List[Dict]:
        fills = []
        ...
        for stop_order in all_stop_orders:
            if hasattr(stop_order, 'is_stop_loss') and stop_order.is_stop_loss:
                if self._is_stop_triggered(stop_order, price_level):
                    activated_time = self._apply_latency(current_time, stop_order.order_id)
                    self.order_manager.activate_stop_order(stop_order, activated_time, self.latency_ms)
                    ...
                    if stop_order.order_type == OrderType.STOP_LIMIT:
                        ...
                        stop_fills = self._execute_stop_market_fallback(stop_order, step_high, step_low, activated_time)
                        fills.extend(stop_fills)

        return fills

    def _process_take_profit_orders(self, price_level: Decimal, step_high: Decimal, step_low: Decimal, current_time: datetime) -> List[Dict]:
        fills = []
    ...
    def _process_other_stop_orders(self, price_level: Decimal, step_high: Decimal, step_low: Decimal, current_time: datetime) -> List[Dict]:
        fills = []
        
        all_stop_orders = self.order_manager.get_active_stop_orders()
        
        for stop_order in all_stop_orders:
            if (not hasattr(stop_order, 'is_stop_loss') or not stop_order.is_stop_loss) and \
               (not hasattr(stop_order, 'is_take_profit') or not stop_order.is_take_profit):
                if self._is_stop_triggered(stop_order, price_level):
                    activated_time = self._apply_latency(current_time, stop_order.order_id)
                    self.order_manager.activate_stop_order(stop_order, activated_time, self.latency_ms)
    ...
    def _process_market_orders_at_level(self, price_level: Decimal, step_high: Decimal, step_low: Decimal, current_time: datetime) -> List[Dict]:
        fills = []
        
        market_orders = []
        while self.order_manager.market_orders:
            activation_time, created_timestamp, order_id = self.order_manager.market_orders[0]
        ...
        for market_order in market_orders:
            ...
            if market_order.side == OrderSide.BUY:
                while (remaining_qty > Decimal('0') and self.order_manager.limit_sells):
                    ...
                    sell_price, activation_time, created_timestamp, order_id = best_sell
                ...
                while (remaining_qty > Decimal('0') and self.order_manager.limit_buys):
                    ...
                    neg_buy_price, activation_time, created_timestamp, order_id = best_buy
            ...
            if remaining_qty > Decimal('0'):
                fallback_fills = self._execute_stop_market_fallback(market_order, step_high, step_low, current_time)
                fills.extend(fallback_fills)
        
        return fills

    def _process_limit_orders_at_level(self, price_level: Decimal, current_time: datetime) -> List[Dict]:
        fills = []
        ...
        while (self.order_manager.limit_buys and self.order_manager.limit_sells):
            ...
            if (buy_id not in self.order_manager.all_orders or 
                sell_id not in self.order_manager.all_orders):
            ...
            if (buy_order.status != OrderStatus.ACTIVE or 
            ...
            if not (self._is_price_touched(buy_order, price_level) and 
                    self._is_price_touched(sell_order, price_level)):
                break
    ...
    def process_bar(self, open_price: Decimal, high: Decimal, low: Decimal, close: Decimal,
                   volume: Decimal, bar_timestamp: datetime, mode: SegmentMode = SegmentMode.TV) -> List[Dict]:
        all_fills = []
        
        self._update_volume_history(volume)
        
        direction = BarDirection.BULLISH if close > open_price else BarDirection.BEARISH
        ...
        if mode == SegmentMode.MICROTICKS:
            sequence = self._get_microtick_sequence(open_price, high, low, close, direction)
        else:
            sequence = self._get_tradingview_sequence(open_price, high, low, close, direction)
        ...
        for price_level, step_high, step_low in sequence:
            ...
            sl_fills = self._process_stop_loss_orders(price_level, step_high, step_low, bar_timestamp)
            level_fills.extend(sl_fills)
            
            tp_fills = self._process_take_profit_orders(price_level, step_high, step_low, bar_timestamp)
            level_fills.extend(tp_fills)
            
            other_stop_fills = self._process_other_stop_orders(price_level, step_high, step_low, bar_timestamp)
            level_fills.extend(other_stop_fills)
            
            market_fills = self._process_market_orders_at_level(price_level, step_high, step_low, bar_timestamp)
            level_fills.extend(market_fills)
            
            limit_fills = self._process_limit_orders_at_level(price_level, bar_timestamp)
    ...
    def get_order_book_snapshot(self) -> Dict[str, Any]:
        ...
        sorted_bids = sorted([(price, data['quantity'], data['orders']) 
                            for price, data in bids.items()], reverse=True)
        sorted_asks = sorted([(price, data['quantity'], data['orders']) 
                            for price, data in asks.items()])
    ...
    def _test_latency_and_queue(self) -> Dict[str, Any]:
        ...
        # Test 1: Latency reproducibility
        ...
        # Test 2: FIFO correctness
        self.queue_mode = QueueMode.FIFO
        order1 = Order(order_id="1", symbol="TEST", side=OrderSide.BUY, quantity=Decimal('100'),
                      order_type=OrderType.LIMIT, limit_price=Decimal('100.00'),
                      created_timestamp=datetime(2024, 1, 1, 10, 0, 0))
        order2 = Order(order_id="2", symbol="TEST", side=OrderSide.BUY, quantity=Decimal('100'),
                      order_type=OrderType.LIMIT, limit_price=Decimal('100.00'),
        ...
        test_results['fifo_correctness'] = aggressor.order_id == "2" and resting.order_id == "1"
        
        # Test 3: Pro-Rata basic functionality
        self.queue_mode = QueueMode.PRO_RATA
        aggressor_order = Order(order_id="A", symbol="TEST", side=OrderSide.BUY, quantity=Decimal('100'),
                              order_type=OrderType.LIMIT, limit_price=Decimal('100.00'))
        resting_order1 = Order(order_id="R1", symbol="TEST", side=OrderSide.SELL, quantity=Decimal('30'),
                              order_type=OrderType.LIMIT, limit_price=Decimal('100.00'))
        resting_order2 = Order(order_id="R2", symbol="TEST", side=OrderSide.SELL, quantity=Decimal('70'),
                              order_type=OrderType.LIMIT, limit_price=Decimal('100.00'))
        
        test_results['pro_rata_available'] = True  # Basic structure test
        
        # Test 4: Bucket snapping
        self.latency_bucket_ms = 10
        test_time = datetime(2024, 1, 1, 10, 0, 0, 123456)  # 123ms
        bucketed_time = self._apply_latency(test_time, "bucket_test")
        test_results['bucket_snapping'] = bucketed_time.microsecond % 10000 == 0  # Should be multiple of 10ms
        
        # Test 5: Zero latency mode
    ...
    def _test_slippage(self) -> Dict[str, Any]:
        ...
        # Test 1: No slippage mode
        original_mode = self.slippage_mode
        self.slippage_mode = "none"
        result_none = self._apply_slippage(Decimal('100.00'), Decimal('1000'), OrderSide.BUY)
        test_results['no_slippage'] = result_none == Decimal('100.00')
        
        # Test 2: Fixed slippage BUY
        self.slippage_mode = "fixed"
        self.fixed_slippage_ticks = Decimal('2')
        result_fixed_buy = self._apply_slippage(Decimal('100.00'), Decimal('1000'), OrderSide.BUY)
        test_results['fixed_buy'] = result_fixed_buy == Decimal('100.02')
        
        # Test 3: Fixed slippage SELL
        result_fixed_sell = self._apply_slippage(Decimal('100.00'), Decimal('1000'), OrderSide.SELL)
        test_results['fixed_sell'] = result_fixed_sell == Decimal('99.98')
        
        # Test 4: Volume slippage
        self.slippage_mode = "volume"
        self.dynamic_coeff = Decimal('10.0')
        self.recent_volumes = [Decimal('1000'), Decimal('2000'), Decimal('1500')]
        result_volume = self._apply_slippage(Decimal('100.00'), Decimal('2000'), OrderSide.BUY)
        avg_volume = Decimal('1500')
        volume_ratio = Decimal('2000') / avg_volume
        expected_ticks = (volume_ratio * self.dynamic_coeff).quantize(Decimal('1'), rounding=ROUND_FLOOR)
        ...
        result1 = self._apply_slippage(Decimal('100.00'), Decimal('1000'), OrderSide.BUY)
        result2 = self._apply_slippage(Decimal('100.00'), Decimal('1000'), OrderSide.BUY)
    ...
    def reset(self):
        self.position = Decimal('0')
        self.recent_volumes = []
        self.rng.seed(42)  # Reset RNG for determinism
